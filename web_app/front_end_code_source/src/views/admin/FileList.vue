...
const preview = (data) => {
  ...
  if (data.status != 2) {
    proxy.Message.warning("File is in transcoding, can't preview");
    return;
  }
...
const delFile = (row) => {
  proxy.Confirm(
    `You really want to delete【${row.fileName}】？you can still recover it in 10 days from recycle bin`,
    async () => {
      let result = await proxy.Request({
        url: api.delFile,
        params: {
...
const delFileBatch = () => {
  ...
  proxy.Confirm(
    `You really want to delete all these files？All deleted files can be recovered from cycle bin in 10 days`,
    async () => {
      let result = await proxy.Request({
        url: api.delFile,
        params: {
