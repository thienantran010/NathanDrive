  ...
  <div>
        ...
        <template #fileName="{ index, row }">
          ...
          >
            ...
            <span class="file-name" v-if="!row.showEdit" :title="row.fileName">
              ...
              <span v-if="row.status == 0" class="transfer-status">in transcoding</span>
              <span v-if="row.status == 1" class="transfer-status transfer-fail"
                >transcoding failed</span
              >
            </span>
    ...
    <div class="no-data" v-else>
      <div class="no-data-inner">
        ...
        <div class="tips">the folder is empty, try upload your first file.</div>
        <div class="op-list">
          <el-upload
            :show-file-list="false"
            :with-credentials="true"
...
const saveNameEdit = async (index) => {
  ...
  if (fileNameReal == "" || fileNameReal.indexOf("/") != -1) {
    proxy.Message.warning("file name should not be null nor containing '/'");
    return;
  }
  let url = api.rename;
  if (fileId == "") {
...
const delFile = (row) => {
  proxy.Confirm(
    `you really want to delete【${row.fileName}】？deleted file can be recovered in 10 days`,
    async () => {
      let result = await proxy.Request({
        url: api.delFile,
        params: {
...
const delFileBatch = () => {
  ...
  proxy.Confirm(
    `you really want to delete these files？deleted files can be recovered in 10 days`,
    async () => {
      let result = await proxy.Request({
        url: api.delFile,
        params: {
...
const moveFolderDone = async (folderId) => {
  if (
    ...
    proxy.Message.warning("files are in current folder, no need to move");
    return;
  }
  let filedIdsArray = [];
...
const preview = (data) => {
  ...
  if (data.status != 2) {
    proxy.Message.warning("file in transcoding, can't preview");
    return;
  }
